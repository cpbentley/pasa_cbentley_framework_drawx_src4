...
public class PassDrawOperator extends AbstractDrwOperator {
   ...
   public void drawMosaic4(GraphicsX g, RgbImage ri, int x, int y, ByteObject mosaic) {
      ...
      boolean trans = mosaic.hasFlag(IBOPassMosaic.PMOSAIC_OFFSET_01_FLAG1, IBOPassMosaic.PMOSAIC_FLAG_1_TRANSFORMATION);
      if (trans) {
         int root = mosaic.get1(IBOPassMosaic.PMOSAIC_OFFSET_03_ROOT1);
      ...
      int overlayW = mosaic.get2(IBOPassMosaic.PMOSAIC_OFFSET_04_OVERLAY_W2);
      int overlayH = mosaic.get2(IBOPassMosaic.PMOSAIC_OFFSET_05_OVERLAY_H2);
      int blend = mosaic.get1(IBOPassMosaic.PMOSAIC_OFFSET_06_BLEND_OVERLAY1);
   ...
   public void drawMosaicGen(GraphicsX g, RgbImage[] ris, int x, int y, ByteObject mosaic) {
      ...
      int overlayW = mosaic.get2(IBOPassMosaic.PMOSAIC_OFFSET_04_OVERLAY_W2);
      int overlayH = mosaic.get2(IBOPassMosaic.PMOSAIC_OFFSET_05_OVERLAY_H2);
      int nw = mosaic.get1(IBOPassMosaic.PMOSAIC_OFFSET_07_NUM_W1);
      int nh = mosaic.get1(IBOPassMosaic.PMOSAIC_OFFSET_08_NUM_H1);
      boolean isdiff = mosaic.hasFlag(IBOPassMosaic.PMOSAIC_OFFSET_01_FLAG1, IBOPassMosaic.PMOSAIC_FLAG_2_DIFF_SOURCES);
      int blend = mosaic.get1(IBOPassMosaic.PMOSAIC_OFFSET_06_BLEND_OVERLAY1);
      BlendOp bo = g.getBlendOp();
      g.setBlendingModeRGB(blend);
   ...
   public void drawMosaic(GraphicsX g, RgbImage[] ri, int x, int y) {
      drawMosaic(g, ri, x, y, drc.getScalerFactory().getMosaic(IBOPassMosaic.PMOSAIC_TYPE_1_SQUARE4, true));
   }

   public void drawMosaic(GraphicsX g, RgbImage ri, int x, int y, ByteObject mos) {
      drawMosaic(g, new RgbImage[] { ri }, x, y, mos);
   }
   ...
   public void drawMosaic(GraphicsX g, RgbImage[] ris, int x, int y, ByteObject mos) {
      RgbImage ri = ris[0];
      int type = mos.get1(IBOPassMosaic.PMOSAIC_OFFSET_02_TYPE1);
      if (type == IBOPassMosaic.PMOSAIC_TYPE_1_SQUARE4) {
         drawMosaic4(g, ri, x, y, mos);
      } else if (type == IBOPassMosaic.PMOSAIC_TYPE_2_SQUARE9) {
   ...
   public void drawSkewBox(GraphicsX g, RgbImage ri, int x, int y, int w, int h, ByteObject skewer) {
      ...
      int op = skewer.get1(IBOPassSkewer.SKEWER_OFFSET_09_BLENDER1);
      GraphicsX gt = g;
      RgbImage rm = null;
      if (op != 0) {
      ...
      int drawOrder = skewer.get1(IBOPassSkewer.SKEWER_OFFSET_10_DRAW_ORDER3);

      RgbImage riLeft = ri;
      RgbImage riFloor = ri;
      if (skewer.hasFlag(IBOPassSkewer.SKEWER_OFFSET_01_FLAG1, IBOPassSkewer.SKEWER_FLAG_6_NO_TRANSFORMS)) {
      ...
      boolean isSymmetric = skewer.hasFlag(IBOPassSkewer.SKEWER_OFFSET_01_FLAG1, IBOPassSkewer.SKEWER_FLAG_7_SYMMETRIC);
      //we must find bet and H based on image with 
      int extraX = skewer.get2(IBOPassSkewer.SKEWER_OFFSET_04_EXTRA_X2);
      int extraY = skewer.get2(IBOPassSkewer.SKEWER_OFFSET_05_EXTRA_Y2);
      ...
      if (!skewer.hasFlag(IBOPassSkewer.SKEWER_OFFSET_01_FLAG1, IBOPassSkewer.SKEWER_FLAG_4_HIDE_LEFT)) {
      ...
      if (!skewer.hasFlag(IBOPassSkewer.SKEWER_OFFSET_01_FLAG1, IBOPassSkewer.SKEWER_FLAG_5_HIDE_RIGHT)) {
      ...
      if (!skewer.hasFlag(IBOPassSkewer.SKEWER_OFFSET_01_FLAG1, IBOPassSkewer.SKEWER_FLAG_2_HIDE_TOP)) {
         //top
      ...
      y3 = ySkewBot;
      //bottom
      if (!skewer.hasFlag(IBOPassSkewer.SKEWER_OFFSET_01_FLAG1, IBOPassSkewer.SKEWER_FLAG_3_HIDE_BOT)) {
         if (isSymmetric) {
      ...
      if (!skewer.hasFlag(IBOPassSkewer.SKEWER_OFFSET_01_FLAG1, IBOPassSkewer.SKEWER_FLAG_1_HIDE_CENTER)) {
         //draw final image
         int centerTrans = skewer.get1(IBOPassSkewer.SKEWER_OFFSET_07_CENTER_TRANS1);
